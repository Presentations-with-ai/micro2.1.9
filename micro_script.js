...
const USER_NAME_BADGE_SUBTITLE = 'временный пользователь';
...
const SUBSCRIPTION_PAYMENT_CONFIG = window.SUBSCRIPTION_PAYMENT_CONFIG || {
  ...
  plans: [
    { title: 'Микроэкономика 2', price: '30 000 сум', description: 'Доступ только к разделу микроэкономики', accessType: PREMIUM_ACCESS_TYPES.MICRO },
    { title: 'Маркетинг', price: '30 000 сум', description: 'Доступ только к разделу маркетинга', accessType: PREMIUM_ACCESS_TYPES.MARKETING },
    { title: 'Оба раздела', price: '50 000 сум', description: 'Микроэкономика 2 + Маркетинг одним ключом', accessType: PREMIUM_ACCESS_TYPES.COMBO }
  ]
};
const BANNED_USERS_CACHE_BUSTER = '2026-04-22-ban-premium-v1';
...
function grantPremiumAccessForUser(userId, details = {}) {
  ...
  store.users[normalizedUserId] = {
    ...
    expiryDate: normalizeActivationDateToken(details?.expiryDate || details?.issuedDate || getTashkentDateToken()),
    version: String(details?.version || 'micro-v2'),
    tokenPreview: String(details?.tokenPreview || '').slice(0, 32),
    accessType,
    sections: getSectionsForAccessType(accessType)
  };
  ...
  if (!document.getElementById('ban-overlay')) {
    ...
    const blockedPlansHtml = (SUBSCRIPTION_PAYMENT_CONFIG?.plans || []).map((plan) => `
      <div class="blocked-plan-card">
        ...
        <div class="blocked-plan-price">${escapeHtml(plan.price || '')}</div>
      </div>
    `).join('');
    ...
    banOverlay.innerHTML = `
      <div class="agreement-panel">
        ...
        <h2 class="agreement-title">Ваш аккаунт заблокирован</h2>
        ...
        <p class="premium-note" data-role="premium-access-note">Если пользователь покупает подписку и активирует ключ, блокировка снимается автоматически для оплаченного раздела.</p>
    ...
    const submitActivation = async () => {
      ...
      grantPremiumAccessForUser(userId, {
      ...
      if (noteNode) {
        noteNode.textContent = `Доступ успешно активирован: ${getPremiumAccessTypeLabel(verification.parsed?.accessType)}.`;
...
function applyPremiumAccessOverlayContent(accessStatus = {}) {
  ...
  if (leadNode) {
    ...
    leadNode.innerHTML = normalizedUserId
      ? `Доступ к ботам для этого Telegram ID ограничен. Для разбана обратитесь к администратору: <a class="blocked-contact-link" href="https://t.me/${adminUrl}" target="_blank" rel="noopener noreferrer">${escapeHtml(admin)}</a>.`
      : 'Telegram ID пока не определён. Откройте приложение через Telegram или нажмите «Обновить ID».';
  }

  if (noteNode) {
    noteNode.textContent = normalizedUserId
      ? 'Если пользователь покупает подписку и активирует ключ, блокировка снимается автоматически для оплаченного раздела.'
...
async function getCurrentBanStatus() {
  ...
  if (hasStoredPremiumAccessForUser(normalizedUserId, CURRENT_QUIZ_SECTION)) {
    ...
    return {
      ...
      availableSections: storedRecord?.sections || getSectionsForAccessType(storedRecord?.accessType)
    };
  }
  ...
  if (accessRecordAllowsSection(premiumRecord, CURRENT_QUIZ_SECTION)) {
    return {
      ...baseStatus,
      accessType: premiumRecord.accessType || PREMIUM_ACCESS_TYPES.COMBO,
      availableSections: premiumRecord.sections || getSectionsForAccessType(premiumRecord.accessType)
    };
  }
  ...
  if (bannedRecord) {
    return {
      ...
      accessType: bannedRecord.accessType || PREMIUM_ACCESS_TYPES.COMBO,
      availableSections: bannedRecord.sections || getSectionsForAccessType(bannedRecord.accessType)
    };
  }
      ...
      <div class="subscription-plans">
        ${plans.map(plan => `
          <div class="subscription-plan-card">
            ...
            <div class="subscription-plan-price">${escapeHtml(plan.price || '')}</div>
